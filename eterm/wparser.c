...
void proceed_w_command(char command, uint16_t timeout) {
	...
	switch(command)
	{
		...
		case 'R':	// enable connect-watchdog
		{
			...
			uint8_t		wdp_code;
			if(timeout >= 2000) {
				real_timeout = 2000; wdp_code = WDTO_2S; }
			else
				if(timeout > 1000) {
					real_timeout = 2000; wdp_code = WDTO_2S; }
				else
					if(timeout > 500) {
						real_timeout = 1000; wdp_code = WDTO_1S; }
					else
						if(timeout > 250) {
							real_timeout = 500; wdp_code = WDTO_500MS; }
						else
							if(timeout > 120) {
								real_timeout = 250; wdp_code = WDTO_250MS; }
							else
								if(timeout > 60) {
									real_timeout = 120; wdp_code = WDTO_120MS; }
								else
									if(timeout > 30) {
										real_timeout = 60; wdp_code = WDTO_60MS; }
									else
										if(timeout > 15) {
											real_timeout = 30; wdp_code = WDTO_30MS; }
										else {
											real_timeout = 15; wdp_code = WDTO_15MS; }
...
static parser_t wcomand[] = {
	PARSER_INIT('W', "W-command", w_parser),
	PARSER_INIT('N', "Reset connection watchdog", n_parser)
};

void register_wcommand(void) {
	register_parser(wcomand + 0);
	register_parser(wcomand + 1);
}
